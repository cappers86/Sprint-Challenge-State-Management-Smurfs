...
- [ ] What problem does the context API help solve?
        The main problem API context helps solve is the isssues which arise with prop drilling.


- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions: actions are payloads which allow us to pass out information from out application store they are the only source of info.

reducers: specify how the applications state changes when an action takes place in our store.

store: The store holds the whole state tree of the application the only way we are able to change this is by dispatching a action. 
...
Redux-thunk is a middleware that lets us call actions creators and return a function instead of a action object. 
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

i think my favourite would have to be connect() because it seemed quite simple to understand and this was the one i felt i got almsot straighht awa. 
